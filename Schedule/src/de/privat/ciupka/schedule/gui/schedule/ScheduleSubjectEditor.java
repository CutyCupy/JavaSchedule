...
public class ScheduleSubjectEditor extends JFrame {
	...
	private void resetComponents(SubjectLabel subject) {
		startHourT.setText(String.valueOf(subject.getStartTime() / 60));
		System.out.println(subject.getStartTime() / 60);
		startMinuteT.setText(String.valueOf(subject.getStartTime() % 60));
		endHourT.setText(String.valueOf(subject.getEndTime() / 60));
		endMinuteT.setText(String.valueOf(subject.getEndTime() % 60));
		endHourT.getText();
		endMinuteT.getText();
		startHourT.getText();
		...
		addB.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if(addB.getText().equals("Add")) {
					...
					if(!isVisible()) {
						guiCon.getSchedule().setEdited(true);
					}
				} else {
					setVisible(false);
	...
	public void setStartTime(int time) {
		startHourT.setText(String.valueOf(time / 60));
		startMinuteT.setText(String.valueOf(time % 60));
		startHourT.focusLost(null);
		startMinuteT.focusLost(null);
	}

	public void setEndTime(int time) {
		endHourT.setText(String.valueOf(time / 60));
		endMinuteT.setText(String.valueOf(time % 60));
		endHourT.focusLost(null);
		endMinuteT.focusLost(null);
	}
